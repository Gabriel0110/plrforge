...
export function LoadoutsPanel({
  ...
  const gearSelected = (selected.area === "loadoutArmor" && selected.loadout === loadout && selected.slot < 10)
    || (selected.area === "loadoutDye" && selected.loadout === loadout);
  const vanitySelected = selected.area === "loadoutArmor" && selected.loadout === loadout && selected.slot >= 10;
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-5">
        ...
        <RovingGroup label="Equipment loadout" role="tablist" activateOnMove className="flex rounded-lg border border-white/10 bg-white/[0.025] p-1">
          {loadoutTabs.map(({ id, index }) => (
            <button
              type="button"
              role="tab"
              data-roving-item=""
              aria-selected={loadout === index}
              aria-controls="loadout-panel"
              tabIndex={loadout === index ? 0 : -1}
              key={id}
      ...
      <div id="loadout-panel" role="tabpanel" aria-label={`Loadout ${loadout + 1}`} className="grid grid-cols-2 gap-5">
        <section className="rounded-xl border border-white/[0.08] bg-white/[0.018] p-4">
          ...
          <KeyboardGrid label={`Loadout ${loadout + 1} gear and dye slots`} columns={2} className="space-y-1.5">
            {gearLabels.map((label, slot) => (
              <div key={label} className="grid grid-cols-[112px_58px_58px_32px] items-center gap-2">
                ...
                <ItemSlotButton keyboardGridItem tabIndex={selectedAt(selected, "loadoutArmor", loadout, slot) || (!gearSelected && slot === 0) ? 0 : -1} item={active.armor[slot]} label={`${label} item`} showIndex={false} selected={selectedAt(selected, "loadoutArmor", loadout, slot)} onSelect={() => onSelect({ area: "loadoutArmor", loadout, slot })} />
                <ItemSlotButton keyboardGridItem tabIndex={selectedAt(selected, "loadoutDye", loadout, slot) ? 0 : -1} item={active.dyes[slot]} label={`${label} dye`} showIndex={false} selected={selectedAt(selected, "loadoutDye", loadout, slot)} onSelect={() => onSelect({ area: "loadoutDye", loadout, slot })} />
        ...
        <section className="rounded-xl border border-white/[0.08] bg-white/[0.018] p-4">
          ...
          <KeyboardGrid label={`Loadout ${loadout + 1} vanity slots`} columns={1} className="space-y-1.5">
            {vanityLabels.map((label, index) => {
              const slot = index + 10;
              return (
                <div key={label} className="grid grid-cols-[124px_58px_1fr] items-center gap-2">
                  <span className="truncate text-[11px] text-white/46">{label}</span>
                  <ItemSlotButton keyboardGridItem tabIndex={selectedAt(selected, "loadoutArmor", loadout, slot) || (!vanitySelected && index === 0) ? 0 : -1} item={active.armor[slot]} label={label} showIndex={false} selected={selectedAt(selected, "loadoutArmor", loadout, slot)} onSelect={() => onSelect({ area: "loadoutArmor", loadout, slot })} />
        ...
        <div className="grid grid-cols-5 gap-3">
          {miscLabels.map((label, slot) => (
            <div key={label} className="rounded-lg border border-white/[0.07] bg-white/[0.015] p-2.5">
              ...
              <KeyboardGrid label={`${label} equipment and dye slots`} columns={2} className="grid grid-cols-2 gap-1.5">
                <ItemSlotButton keyboardGridItem tabIndex={!selectedAt(selected, "miscDye", undefined, slot) ? 0 : -1} item={equipment.miscEquips[slot]} label={label} showIndex={false} selected={selectedAt(selected, "miscEquip", undefined, slot)} onSelect={() => onSelect({ area: "miscEquip", slot })} />
                <ItemSlotButton keyboardGridItem tabIndex={selectedAt(selected, "miscDye", undefined, slot) ? 0 : -1} item={equipment.miscDyes[slot]} label={`${label} dye`} showIndex={false} selected={selectedAt(selected, "miscDye", undefined, slot)} onSelect={() => onSelect({ area: "miscDye", slot })} />
              </KeyboardGrid>
            </div>
          ))}
        </div>
