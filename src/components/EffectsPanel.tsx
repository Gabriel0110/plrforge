...
import { BuffGlyph } from "./ItemGlyph";
...
export function EffectsPanel({ effects, onChange }: Props) {
  ...
  const [activeBuffId, setActiveBuffId] = useState<number | null>(null);
  ...
  const matchedActiveIndex = results.findIndex((record) => record.id === activeBuffId);
  ...
  return (
    <main className="min-h-0 overflow-y-auto px-7 py-6">
      <div className="mx-auto max-w-[1040px]">
        <div className="flex items-end justify-between gap-5"><div><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-emerald-300/65">Saved effects</p><h1 className="mt-2 text-2xl font-semibold tracking-[-0.045em] text-white/92">Buffs and durations</h1><p className="mt-2 max-w-[68ch] text-sm leading-6 text-white/38">Terraria player files store 44 effect slots and measure duration in 60 ticks per second. Transient effects marked no-save by Terraria may be discarded the next time the game saves.</p></div><div className="rounded-lg border border-white/[0.08] bg-black/15 px-3 py-2 text-right"><p className="font-mono text-[9px] uppercase tracking-[0.12em] text-white/28">Slots used</p><p className="mt-1 font-mono text-xs text-white/65">{active.length} / 44</p></div></div>

        <section className="relative mt-6 rounded-xl border border-white/[0.08] bg-white/[0.018] p-5">
          <label className="relative block"><MagnifyingGlass className="pointer-events-none absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-white/36" /><input aria-label="Find a buff by name or ID" role="combobox" aria-autocomplete="list" aria-expanded={Boolean(query)} aria-controls={listboxId} aria-activedescendant={query && results[activeIndex] ? `${listboxId}-${results[activeIndex].id}` : undefined} value={query} onChange={(event) => { setActiveBuffId(null); setQuery(event.target.value); }} onKeyDown={(event) => { if (event.key === "ArrowDown" && results.length) { event.preventDefault(); setActiveBuffId(results[Math.min(results.length - 1, activeIndex + 1)].id); } else if (event.key === "ArrowUp" && results.length) { event.preventDefault(); setActiveBuffId(results[Math.max(0, activeIndex - 1)].id); } else if (event.key === "Enter" && results[activeIndex]) { event.preventDefault(); addBuff(results[activeIndex]); } else if (event.key === "Escape" && query) { event.preventDefault(); setActiveBuffId(null); setQuery(""); } }} placeholder="Find a buff by name or ID" className="h-11 w-full rounded-xl border border-white/10 bg-black/20 pl-10 pr-4 text-sm text-white/82 placeholder:text-white/28 focus:border-emerald-300/45" /></label>
          {query && <div id={listboxId} role={results.length ? "listbox" : "status"} aria-label={results.length ? "Buff search results" : undefined} className="absolute left-5 right-5 top-[72px] z-20 overflow-hidden rounded-xl border border-white/12 bg-[#181d1b] shadow-[0_20px_50px_rgba(0,0,0,.5)]">{results.length ? results.map((record, index) => <button type="button" role="option" id={`${listboxId}-${record.id}`} aria-selected={index === activeIndex} tabIndex={-1} key={record.id} disabled={active.length === 44} onClick={() => addBuff(record)} className={`flex w-full items-center gap-3 border-b border-white/[0.06] px-4 py-3 text-left last:border-0 hover:bg-white/[0.05] ${index === activeIndex ? "bg-white/[0.05]" : ""}`}><BuffGlyph buffId={record.id} /><span className="min-w-0 flex-1"><span className="block text-[12px] font-medium text-white/80">{record.name}</span><span className="mt-0.5 block truncate text-[10px] text-white/32">{record.description || record.key}</span></span><Plus className="size-4 text-emerald-300/70" /></button>) : <p className="p-4 text-sm text-white/42">No buff matches that name or ID.</p>}</div>}
        </section>
