...
export function ItemSearch({ query, onQueryChange, onChoose, targetLabel, acceptItem, onBrowse }: Props) {
  ...
  const matchedActiveIndex = results.findIndex((item) => item.id === activeItemId);
    ...
    <div className="relative flex gap-2">
      ...
      <input
        ...
        onKeyDown={(event) => {
          if (event.key === "ArrowDown" && results.length) {
            ...
            setActiveItemId(results[Math.min(results.length - 1, activeIndex + 1)].id);
          } else if (event.key === "ArrowUp" && results.length) {
            event.preventDefault();
            setActiveItemId(results[Math.max(0, activeIndex - 1)].id);
          } else if (event.key === "Home" && results.length) {
            event.preventDefault();
            setActiveItemId(results[0].id);
          } else if (event.key === "End" && results.length) {
            event.preventDefault();
            setActiveItemId(results.at(-1)!.id);
        ...
        aria-activedescendant={query && results[activeIndex] ? `${listboxId}-${results[activeIndex].id}` : undefined}
      ...
      {query && (
        <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-20 overflow-hidden rounded-xl border border-white/12 bg-[#181d1b] shadow-[0_22px_60px_-20px_rgba(0,0,0,.72)]">
          <div aria-live="polite" className="flex items-center justify-between border-b border-white/[0.08] px-3.5 py-2 text-[10px] font-medium uppercase tracking-[0.12em] text-white/34">
            <span>{results.length ? `${results.length} closest matches` : "No catalog match"}</span>
            <span>{targetLabel}</span>
          </div>
          {results.length ? (
            <div id={listboxId} role="listbox" aria-label={`Items for ${targetLabel}`} className="max-h-[330px] overflow-y-auto p-1.5">
              {results.map((item) => (
                <ItemTooltip key={item.id} itemId={item.id} context="Search result">
                  {(tooltipProps) => <button
                    ...
                    className={`flex w-full items-center gap-3 rounded-lg px-2.5 py-2 text-left transition hover:bg-white/[0.065] active:scale-[0.995] ${results[activeIndex]?.id === item.id ? "bg-white/[0.065]" : ""}`}
                  >
            ...
            <div id={listboxId} role="status" className="px-4 py-5">
